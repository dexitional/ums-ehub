  ...
  return (
    <div className="px-3 md:px-6 pb-4 grid md:grid-cols-7 gap-y-4 md:gap-y-0 md:gap-x-2 md:place-items-center text-gray-500 border-b border-slate-200 hover:bg-slate-50/50 group">
        ...
        <div className="capitalize flex flex-col space-y-2">
            ...
            <span className="px-2">{data?.phase?.project?.title}</span>
        </div>
        <div className="flex flex-col space-y-2">
            <ListHeading title="Phase" />
            <span className="px-2 ">{data?.phase?.title}</span>
        </div>
        <div className="flex flex-col space-y-2">
            <ListHeading title="Amount" />
            <div className="px-2 flex flex-col md:text-center space-y-1 ">
               <span className="text-[0.8rem]">{data?.currency}</span>
               <span className="text-xs">{data?.paidAmount}</span>
            </div>
        </div>
        <div className="flex flex-col space-y-2">
