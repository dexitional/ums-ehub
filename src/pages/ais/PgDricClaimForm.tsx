...
export async function action({ request, params }){
   const id = params?.claimId || 0;
   const formData = await request.formData()
   const data = Object.fromEntries(formData)
   let resp;
    if(id != 0) 
      resp = await Service.updateClaim(id,data);
    else
      resp = await Service.postClaim(data);
   
    if(resp){
      return redirect(`/dric/claims/${resp.id}`)
    }
} 
...
function PgDricClaimForm({}: Props) {
...
export default PgDricClaimForm