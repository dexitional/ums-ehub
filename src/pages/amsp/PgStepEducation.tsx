...
function PgStepEducation({}: Props) {
  ...
  const { data,applicant,institutecats,certcats,stepUrl,awardclasses }: any = useLoaderData();
                ...
                <div className=" w-full grid grid-cols-1 md:grid-cols-2 gap-y-2 md:gap-x-4 ">
                  <div className="px-2 space-y-4">
                      ...
                      <label className="flex flex-col space-y-2">
                          ...
                          <select arial-label="classValue" name={`classValue_${i}`} defaultValue={rec?.classValue} className="capitalize focus:ring-0 border focus:border-slate-300  border-slate-200 bg-blue-500/5 text-sm md:text-base text-gray-500 rounded-md">
                            <option selected disabled>-- Choose --</option>
                            { awardclasses?.map((r) => <option value={r?.id}>{r?.title?.toUpperCase()}</option>)}
                          </select>
                      </label>
