        ...
        <section className="m-2 my-1 md:my-10 md:mx-4 md:w-[28rem] rounded-b-xl md:rounded-xl border-[3px] border-primary-dark/20 backdrop-blur-lg bg-primary-dark/20 bg-[url('./assets/img/eagle.png')] bg-no-repeat bg-bottom flex flex-col justify-between overflow-y-scroll scrollbar-hide">
            <div className="p-6 flex-1 flex flex-col items-center">
                ...
                <div className="my-6 md:my-14 w-full space-y-8">
                    {/* Message */}
                    <div className={`${message ? 'flex':'hidden'} p-3 md:p-4 mx-auto md:w-[90%] md:space-x-4 shadow rounded-xl backdrop-blur-sm bg-yellow-100 bg-opacity-70`}>
                        <MdWarning className="h-10 w-10 text-yellow-50"/>
                        <p className="font-medium text-gray-600">Something went wrong while attempting to validate your credentials.</p>
                    ...
                    <form onSubmit={authenticateCredential} className={`${showStudentForm || showSSOForm ? 'flex':'hidden'} mx-auto py-4 px-3 md:p-4 md:w-[90%] rounded-xl border-[3px] border-primary/20 bg-primary/5 flex-col space-y-4 text-white text-lg`}>
                        ...
                        <button onClick={(e)=> { e.preventDefault(); setShowStudentForm(false); setShowSSOForm(false); }} className="py-1 px-4 mx-auto w-fit rounded-xl bg-black/10 focus:ring-0 focus:outline-none flex items-center space-x-2">
                          <FaArrowLeft className="w-3 h-3 text-[#ffee1f]" />
                          <span className="text-sm font-bold text-[#ffee1f]">Go Back</span>
                        </button>
                    </form>
                </div>
